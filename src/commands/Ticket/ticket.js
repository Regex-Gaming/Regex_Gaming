...
export default {
    ...
    async execute(interaction, config, client) {
        ...
        if (subcommand === "rename") {
            ...
            // Optional check: Ensure the command is executed in a text channel
            if (!channel || channel.type !== ChannelType.GuildText) {
                return await replyUserError(interaction, {
                    type: ErrorTypes.UNKNOWN,
                    message: "This command can only be used inside a text channel."
                });
            }
            ...
            try {
                ...
                return await InteractionHelper.safeEditReply(interaction, {
                    embeds: [
                        successEmbed(
                            "Channel Renamed",
                            `Successfully renamed channel from **#${oldName}** to **#${newName}**.`
                        )
                    ]
                });
            } catch (error) {
                ...
                return await replyUserError(interaction, {
                    type: ErrorTypes.UNKNOWN,
                    message: 'Failed to rename the channel. Make sure the bot has the `Manage Channels` permission and rate limits are not exceeded.'
                });
            }
        }
